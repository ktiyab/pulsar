...
# Remove deployment command: ./remove.sh "<APP-NAME>" "<PROJECT-ID>" "<REGION>"
...
if [ -z "$2" ]
  then
    ...
    echo "---> The app will be removed from the GCP project: "$2
    PROJECT_ID=$2
fi
...
if [ -z "$3" ]
  then
    ...
    echo "---> Custom region is specified, the process  will use region: ${3}"
    REGION=$3
fi
