...
class BigqueryResource(object):
    ...
    mailing_html_template = "<b>Doberman</b> identify a BigQuery item compliance violation with following details: " \
                            " <br/><pre>{}</pre>." \
    ...
    def datasetservice_insert(payload):
        ...
        project_name_array = payload["projectId"].split("-")

        if len(project_name_array) == 3:
            # 2 - Get alpha code
            country_alpha_code = project_name_array[2]
            # 3 - If we have valid alpha-code
            if country_alpha_code in BigqueryResource.ALLOWED_COUNTRY_ALPHA_CODES:
                # 4 - If current location is not in the valid location list
                if payload["currentLocations"] not in BigqueryResource.rules[country_alpha_code.upper()]:
                    # Houston, we have a problem
                    subject = BigqueryResource.mailing_subject.format(payload["projectId"])
                    body = BigqueryResource.mailing_html_template.format(json.dumps(payload, indent=4),
                                                                         payload["projectId"],
                                                                         str(BigqueryResource.rules[
